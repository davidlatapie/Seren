...
{
    return min(nIntervalEnd - nIntervalBeginning - GetMinStakeAge(nIntervalEnd), (int64)nStakeMaxAge);
}
...
static bool GetKernelStakeModifier(uint256 hashBlockFrom, uint64& nStakeModifier, int& nStakeModifierHeight, int64& nStakeModifierTime, bool fPrintProofOfStake)
{
    ...
    while (nStakeModifierTime < pindexFrom->GetBlockTime() + nStakeModifierSelectionInterval)
    {
        ...
        {   // reached best block; may happen if node is behind on block chain
            if (fPrintProofOfStake || (pindex->GetBlockTime() + GetMinStakeAge(pindex->GetBlockTime()) - nStakeModifierSelectionInterval > GetAdjustedTime()))
...
{	
	...
	int64 nTimeWeight = GetWeight((int64)nIntervalBeginning, (int64)nIntervalEnd);
	
	//return false if nTimeWeight is negative
	if(nTimeWeight & 0x80)
		return false;
		
	CBigNum bnCoinDayWeight = CBigNum(nValueIn) * nTimeWeight / COIN / (24 * 60 * 60) * 100; // multiply by 100 to give extra sensitivity for a low money supply coin
	...
	const COutPoint& prevout, unsigned int& nTimeTx, unsigned int nHashDrift, bool fCheck, uint256& hashProofOfStake, bool fPrintProofOfStake)
{
	...
	if (nTimeBlockFrom + GetMinStakeAge(nTimeTx + 60) > nTimeTx) // Min age requirement, give 60 second addition to nTimeTx because it could hash that far into future
	...
	//grab stake modifier - Seren improves hashing by only grabbing this once per utxo
	...
	if(fCheck)
	{
		...
		return stakeTargetHit(hashProofOfStake, nTxPrevTime, nTimeTx, nValueIn, bnTargetPerCoinDay);
	}
	...
	for(i = 0; i < (nHashDrift); i++) //iterate the hashing
	{
		...
		if(!stakeTargetHit(hashProofOfStake, nTxPrevTime, nTimeTx, nValueIn, bnTargetPerCoinDay))
			continue;
