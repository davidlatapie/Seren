...
unsigned int nStakeMinAgeFastPoS = 60 * 60 * 3; // 3 hour stake maturity until after the FAST_POS_PERIOD_END in order for smoother transition from PoW to PoS
unsigned int nStakeMinAge = 60 * 60 * 24 * 30;	// after 30 days weight begins to build
unsigned int nStakeMaxAge = 60 * 60 * 24 * 90;	// weight stops building at 90 days
unsigned int nStakeTargetSpacing = 60 * 3;	// 3 minute block spacing

int64 GetMinStakeAge(int64 nTime)
{
	int64 nAgeReturn = 0;
	if(nTime < FAST_POS_PERIOD_END)
		nAgeReturn = nStakeMinAgeFastPoS;
	else 
		nAgeReturn = nStakeMinAge;
...
const string strMessageMagic = "Seren Signed Message:\n";
...
bool CTransaction::CheckTransaction() const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
		...
		if(!IsCoinBase() && !IsCoinStake())
		{
			...
			{
				printf("CheckTransaction(): Could not find previous transaction \n");
				return false;
			}
	...
	int64 nFeePaid = nValueIn - nValueOut;
	if(!IsCoinBase() && !IsCoinStake() && GetMinFee() > nFeePaid)
	{
		printf("CheckTransaciton(): Not enough fee paid \n");
		return false;
	}
...
int64 CTransaction::GetValueInForValueBasedFee() const //presstab
{
	std::map<CTxDestination, int64> mapInAmounts;
	int64 nValueInValueBasedFee = 0;
	BOOST_FOREACH(const CTxIn& txin, vin)
	{
		//search disk for VIN info
		uint256 hashBlockPrev;
		...
		if(!GetTransaction(txin.prevout.hash, txPrev, hashBlockPrev))
		{
			printf("GetValueInForValueBasedFee(): could not find VIN \n");
			return 10000000 * COIN; //return an amount that will yield the max fee if the VIN is not found
		}
	...
	BOOST_FOREACH(const CTxOut txout, vout)
	{
		...
		if(mapInAmounts.count(outAddress))
			...
			nValueInValueBasedFee += txout.nValue;
	}
	
	return nValueInValueBasedFee;
}
                              ...
                              enum GetMinFee_mode mode, unsigned int nBytes) const
{
	...
	int64 nNormalFee = max(nMinFee, MIN_TX_FEE);
	int64 nValueBasedFee = this->GetValueBasedFee();
	
    return nNormalFee + nValueBasedFee;
}
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime, int nHeight)
{
	...
	CBigNum bnSubsidy = CBigNum(nCoinAge) * nRewardCoinYear / 365;
	int64 nSubsidy = bnSubsidy.getuint64();
	
	if (fDebug && GetBoolArg("-printcreation"))
        printf("GetProofOfStakeReward(): create=%s nCoinAge=%"PRI64d" nBits=%d\n", FormatMoney(nSubsidy).c_str(), nCoinAge, nBits);
	
	nSubsidy = max(nSubsidy, (int64)1);
	
    return nSubsidy;
}

static const int64 nTargetTimespan = 60 * 60;
static const int64 nTargetSpacingWorkMax = 180; // 3 minute time span for PoW 
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake) 
{
	...
	// if the spacing is negative, change it to positive to prevent timewarp troubles
	if(nActualSpacing < 0)
		nActualSpacing = 1;
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Seren
    bool fStrictPayToScriptHash = true; // Always active in Seren
...
bool CTransaction::GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        if (block.GetBlockTime() + GetMinStakeAge(block.GetBlockTime()) > nTime)
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
	...
	// presstab Seren: enable of disable staking based on block difficulty
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Seren", CClientUIInterface::OK | CClientUIInterface::ICON_ETRKLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Terrorist who killed Irish cameraman Simon Cumbers executed by Saudi Arabia - Independent.ie";
		if(fTestNet)
			pszTimestamp = "Seren Testnet 12-22-2015";
		
        CTransaction txNew;
        txNew.nTime = 1451838218;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1451838218;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1531984;
        if(fTestNet)
		{
			block.nTime = 1450811328;
			block.nNonce = 32692;
		}
        ...
        assert(block.hashMerkleRoot == (!fTestNet ? uint256("61132cddfd2b367f13228c2cb648d9d8ce1e0a0cedd0a0c973a5771383bb556c") : uint256("03283a26f357cc8ed8be11092eeaf24bbe6ee047096bce100533ad59587a4d5f")));
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    if (fProofOfStake && !pwalletMain->fDisableStake)  // attempt to find a coinstake && make sure settings allow PoS (presstab Seren)
    {
