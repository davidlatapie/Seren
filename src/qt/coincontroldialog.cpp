...
int CoinControlDialog::splitBlockCount;
bool CoinControlDialog::fReturnChange;
...
{
	double dUserAmount = ui->lineEditCustomCC->text().toDouble();
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    {
        std::pair<QString, qint64> amount = CoinControlDialog::payAmounts[i];
		nPayAmount += amount.second;

        if (amount.second > 0)
        {
            if (amount.second < CENT)
                fLowOutput = true;

            CScript scriptPubKey;
            scriptPubKey.SetDestination(CBitcoinAddress(amount.first.toStdString()).Get());

            CTxOut txout(amount.second, scriptPubKey);
    ...
    BOOST_FOREACH(const COutput& out, vOutputs)
    {
        ...
        txDummy.vin.push_back(CTxIn(out.tx->GetHash(), out.i));
    }
    ...
    // add change to txdummy
    if(nAmount - nPayAmount > 0)
    {
        if(CoinControlDialog::coinControl->fReturnChange)
        {
            //assign the change to one of our input addresses so that the fee is calculated correctly
            CScript scriptPubKey;
            scriptPubKey.SetDestination(inputAddress);
            CTxOut txout(nAmount - nPayAmount, scriptPubKey);
            txDummy.vout.push_back(txout);
        }
		//TODO add custom change address logic for fee
        else
        {
            //assign change to an address not owned by us
    ...
    {
        // Bytes
		int nSplitBlockCount = CoinControlDialog::splitBlockCount;
		if(nSplitBlockCount <= 1)
			nSplitBlockCount = 0;
        nBytes = nBytesInputs + ((CoinControlDialog::payAmounts.size() > 0 ? CoinControlDialog::payAmounts.size() + nSplitBlockCount : 2) * 34) + 10;
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(PAIRTYPE(QString, vector<COutput>) coins, mapCoins)
    {
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
			...
			model->getStakeWeightFromValue(out.tx->GetTxTime(), out.tx->vout[out.i].nValue, nTxWeight);
			double dStakeAge = GetMinStakeAge(GetAdjustedTime());
			...
			// Potential Stake
			uint64 nCoinAge;
			out.tx->GetCoinAge(txdb, nCoinAge);
			double nPotentialStake = GetProofOfStakeReward(nCoinAge, pindexBest->nBits, GetTime(), nBestHeight);
			...
			uint64 nBlockWeight = qMax(nTxWeight, uint64(nBlockSize * (dStakeAge/(60*60*24))));
			...
			uint64 nEstimateTime = 180 * nNetworkWeight / nBlockWeight / nAccuracyAdjustment; // 180 seconds is block target
