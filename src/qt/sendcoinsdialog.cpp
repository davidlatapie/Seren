    ...
    model(0)
{
    ...
    ui->sendButton->setIcon(QIcon());
     ...
     ui->lineEditCoinControlChange->setPlaceholderText(tr("Enter a Seren address (e.g. pAvP3gYfuyDVbUt98ToMbwU9rwEdBV1dHW)"));
...
void SendCoinsDialog::clearSplitBlock()
{
	ui->splitBlockLineEdit->setText(QString::number(0));
	ui->labelBlockSize->setText(QString::number(0));
	CoinControlDialog::splitBlockCount = int(0);
}
...
void SendCoinsDialog::splitBlockLineEditChanged(const QString & text)
{
	...
	CoinControlDialog::splitBlockCount = int(text.toDouble());
	coinControlUpdateLabels();
}
 ...
 void SendCoinsDialog::coinControlChangeEdited(const QString & text)
 {
     if (model)
     {
         ...
         else if (!CBitcoinAddress(text.toStdString()).IsValid())
         {
             ...
             ui->labelCoinControlChangeLabel->style()->polish(ui->labelCoinControlChangeLabel);
             ui->labelCoinControlChangeLabel->setText(tr("WARNING: Invalid Seren address"));
         }
         else
         {
 ...
 void SendCoinsDialog::coinControlUpdateLabels()
 {
    ...
    for(int i = 0; i < ui->entries->count(); ++i)
    {
        ...
        if(entry)
        {
            std::pair<QString, qint64> pAmount;
            pAmount.first = entry->getValue().address;
            pAmount.second = entry->getValue().amount;
            CoinControlDialog::payAmounts.append(pAmount);
        }
    }
