    ...
    rpcConsole(0)
{
    ...
    QString windowTitle = tr("Seren") + " - " + tr("Wallet");
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a Seren address"));
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About Seren"), this);
    aboutAction->setToolTip(tr("Show information about Seren"));
    ...
    optionsAction->setToolTip(tr("Modify configuration options for Seren"));
...
void BitcoinGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
    ...
    QString toolTip = tr("Seren client") + " " + networkStyle->getTitleAddText();
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Seren network", "", count));
}
...
void BitcoinGUI::setNumBlocks(int count, int nTotalBlocks)
{
    ...
    if (!clientModel || clientModel->getNumConnections() == 0)
    {
        progressBarLabel->setVisible(false);
        progressBar->setVisible(false);
		labelBlocksIcon->setPixmap(QIcon(":/icons/unknown").pixmap(STATUSBAR_ICONSIZE, STATUSBAR_ICONSIZE));
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Seren address or malformed URI parameters."));
    }
