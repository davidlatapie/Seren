...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "Returns a new Seren address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current Seren address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <Serenaddress> <account>\n"
            "Sets the account associated with the given address.");

    CBitcoinAddress address(params[0].get_str());
    if (!address.IsValid())
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Seren address");
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
		"sendtoaddress <Serenaddress> <amount> <return change>[comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001\n"
			"<return change> true/false, will return the change to one of the output addresses\n"
	...
	// Return Change
	bool fReturnChange = false;
	if(params.size() > 2)
		fReturnChange = params[2].get_bool();
    ...
    string strError = pwalletMain->SendMoneyToDestination(address.Get(), nAmount, wtx, false, false, fReturnChange);
    if (strError != "")
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <Serenaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <Serenaddress> <signature> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "getreceivedbyaddress <Serenaddress> [minconf=1]\n"
            "Returns the total amount received by <Serenaddress> in transactions with at least [minconf] confirmations.");
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
		"sendfrom <fromaccount> <toSerenaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase());
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    BOOST_FOREACH(const Pair& s, sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Seren address: ")+s.name_);
    ...
    bool fCreated = pwalletMain->CreateTransaction(vecSend, wtx, keyChange, nFeeRequired, 1, false);
    if (!fCreated)
    {
...
Value addmultisigaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg = "addmultisigaddress <nrequired> <'[\"key\",\"key\"]'> [account]\n"
            ...
            "each key is a Seren address or hex-encoded public key\n"
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; Seren server stopping, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress <Serenaddress>\n"
            "Return information about <Serenaddress>.");
...
Value validatepubkey(const Array& params, bool fHelp)
{
    if (fHelp || !params.size() || params.size() > 2)
        throw runtime_error(
            "validatepubkey <Serenpubkey>\n"
            "Return information about <Serenpubkey>.");
...
/** Seren Specific RPC Wallet Additions**/
...
double getWeight()
{
	...
	BOOST_FOREACH(const COutput& out, vCoins)
    {
		...
		if(nAge < GetMinStakeAge(GetTime()))
			nWeight = 0;
		nWeightSum += nWeight;
	}
...
Value cclistcoins(const Array& params, bool fHelp)
{
	...
	BOOST_FOREACH(const COutput& out, vCoins)
    {
		...
		uint64 nWeight = 0;
		pwalletMain->GetStakeWeightFromValue(out.tx->GetTxTime(), out.tx->vout[out.i].nValue, nWeight);
		if(dAge < GetMinStakeAge(GetTime()))
			nWeight = 0;
...
Value cccustomchange(const Array& params, bool fHelp)
{
    ...
    if(!address.IsValid()) throw JSONRPCError(RPC_INVALID_PARAMETER, "Invalid Seren address");
...
Value ccsend(const Array& params, bool fHelp)
{
   ...
   vector<pair<CScript, int64> > vecSend;
    ...
    bool fCreated = pwalletMain->CreateTransaction(vecSend, wtx, keyChange, nFeeRequired, 1, false, false, coinControl); // 1 = no splitblock, false for return change, false for s4c, coinControl
    if (!fCreated)
    {
...
Value multisend(const Array &params, bool fHelp)
{
	...
	if (fHelp || params.size() != 2)
        throw runtime_error(
			...
			"multisend <Seren Address> <percent>\n"
