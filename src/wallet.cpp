...

    {
	     ...
	     BOOST_FOREACH(const COutput& out, vCoins)
		{
			...
			if (out.tx->IsCoinStake() && out.tx->GetBlocksToMaturity() == 0  && out.tx->GetDepthInMainChain() == nCoinbaseMaturity+20)
			{
				...
				bool fCreated = CreateTransaction(vecSend, wtx, keyChange, nFeeRet, 1, false, true, cControl);
				if (!fCreated)
...
bool CWallet::SelectStakeCoins(std::set<std::pair<const CWalletTx*,unsigned int> >& setCoins, int64 nTargetAmount) const
{
	...
	BOOST_FOREACH(const COutput& out, vCoins)
	{
		...
		{
			if(GetTime() - out.tx->GetTxTime() > GetMinStakeAge(GetTime()))
			{
...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64> >& vecSend, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet, int nSplitBlock, bool fReturnChange, bool fAllowS4C, const CCoinControl* coinControl)
{
    int64 nValue = 0;
		...
		
    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
				...
				//add a dummy transaction so that we can get the required fee without pushing information onto our real transaction
				CWalletTx txDummy = wtxNew;
				if(fReturnChange || (coinControl && coinControl->fReturnChange == true))
					nFeeRet = txDummy.GetMinFee(1, false, GMF_SEND, nBytes);
				else
				{
					txDummy.vout.push_back(CTxOut(nValueIn - nValue, (CScript)vector<unsigned char>(24, 0))); // change will be charged fee
					nFeeRet = txDummy.GetMinFee(1, false, GMF_SEND, nBytes);
				}
...
bool CWallet::GetStakeWeightFromValue(const int64& nTime, const int64& nValue, uint64& nWeight)
{
	...
	CBigNum bnCoinDayWeight = CBigNum(nValue) * nTimeWeight / COIN / (24 * 60 * 60) * 100; // multiply by 100 to give extra sensitivity for a low money supply coin
	nWeight = bnCoinDayWeight.getuint64();
	return true;
}

bool CWallet::CreateTransaction(CScript scriptPubKey, int64 nValue, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet, bool fReturnChange, bool fAllowS4C, const CCoinControl* coinControl)
{
    vector< pair<CScript, int64> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
    return CreateTransaction(vecSend, wtxNew, reservekey, nFeeRet, 1, fReturnChange, fAllowS4C, coinControl);
}
...
bool CWallet::GetStakeWeight(const CKeyStore& keystore, uint64& nMinWeight, uint64& nMaxWeight, uint64& nWeight, uint64& nHoursToMaturity, uint64& nAmount)
{
	...
	uint64 nStakeAge = GetMinStakeAge(GetTime());
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
        ...
        CBigNum bnCoinDayWeight = CBigNum(pcoin.first->vout[pcoin.second].nValue) * nTimeWeight / COIN / (24 * 60 * 60) * 100; // multiply by 100 to give extra sensitivity for a low money supply coin
...
{
    //presstab Seren: return false if disablestake RPC is enabled
	if(fDisableStake)
		return false;
    ...
    // presstab Seren - Initialize as static and don't update the set on every run of CreateCoinStake() in order to lighten resource use
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setStakeCoins)
    {
		...
		//Seren now iterates each utxo inside of CheckStakeKernelHash()
		if (CheckStakeKernelHash(nBits, block, txindex.pos.nTxPos - txindex.pos.nBlockPos, *pcoin.first, prevoutStake, txNewTime, nHashDrift, false, hashProofOfStake))
		{
			...
			//presstab - calculate the total size of our new output including the stake reward so that we can use it to decide whether to split the stake outputs
			uint64 nCoinAge;
			CTxDB txdb("r");
			const CBlockIndex* pIndex0 = GetLastBlockIndex(pindexBest, false);
	...
	if(fCombineDust) //presstab Seren - this combination code iterates through all of your outputs on successful coinstake, so its useful to have user be able to choose whether this is necessary
	{
		BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setStakeCoins)
		{
...
string CWallet::SendMoney(CScript scriptPubKey, int64 nValue, CWalletTx& wtxNew, bool fAskFee, bool fAllowS4C, bool fReturnChange)
{
    ...
    if (!CreateTransaction(scriptPubKey, nValue, wtxNew, reservekey, nFeeRequired, fReturnChange, fAllowS4C))
    {
        string strError;
...
string CWallet::SendMoneyToDestination(const CTxDestination& address, int64 nValue, CWalletTx& wtxNew, bool fAskFee, bool fAllowStakeForCharity, bool fReturnChange)
{
    // Check amount
    if (nValue <= 0)
    ...
    return SendMoney(scriptPubKey, nValue, wtxNew, fAskFee, fAllowStakeForCharity, fReturnChange);
}
