...
*Setup instructions for a gitian build of Seren using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Seren
...
Clone the git repositories for Seren and gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/seren/seren
```
...
Follow the instructions in [doc/release-process.md](release-process.md) in the seren repository
...
To build Seren (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the seren repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/seren/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/seren/seren
...
For example:
```bash
URL=https://github.com/robvanmieghem/seren.git
COMMIT=master
./bin/gbuild --commit seren=${COMMIT} --url seren=${URL} ../seren/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit seren=${COMMIT} --url seren=${URL} ../seren/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit seren=${COMMIT} --url seren=${URL} ../seren/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/seren-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/seren-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/seren-osx-build.assert
```
...
[seren/gitian.sigs](https://github.com/seren/gitian.sigs/) repository, or if that's not possible create a pull
